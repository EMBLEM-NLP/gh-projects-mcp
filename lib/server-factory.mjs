/**
 * gh-projects-mcp — server/tool construction shared by every transport.
 *
 * `createMcpServer()` builds one fully-registered `McpServer` — the exact
 * same domain/tool registrations regardless of caller — so each transport
 * entrypoint (stdio: server.mjs; remote HTTP: lib/http-transport.mjs, #60)
 * connects an identical server to a different `Transport`. Nothing here is
 * transport-specific except the `transport` label threaded into
 * `gh_preflight` (lib/tools-preflight.mjs, #64) so callers can tell which
 * transport served a given response.
 *
...
import { registerViewTools } from './tools-views-graphql.mjs';
import { VIEW_SELECTION, normalizeView } from './view-api.mjs';
import { registerPrTools } from './tools-pr.mjs';
import { registerIssueLifecycleTools } from './tools-issue-lifecycle.mjs';
import { registerWorkflowTools } from './tools-workflows.mjs';
import { registerPreflightTool } from './tools-preflight.mjs';
import { gqlStr, makeOwnerRoot, assertConfirmed } from './helpers.mjs';
import { createProjectField, updateSelectFieldOptions, configureIterationField, updateMultiSelectItemField } from './field-mutations.mjs';

const SERVER_VERSION = '1.9.0';

/**
 * Build one fully-registered `McpServer` instance. Callers connect it to
 * whichever `Transport` fits their runtime (`StdioServerTransport` for
 * stdio, `StreamableHTTPServerTransport` for remote HTTP) — see
 * server.mjs and lib/http-transport.mjs.
 *
 * @param {{ transport?: string }} [options] `transport` is only used to
 *   label the `gh_preflight` report (e.g. "stdio" or "http"); it has no
 *   effect on which tools are registered or how they behave.
 * @returns {McpServer}
 */
export function createMcpServer({ transport = 'stdio' } = {}) {
const server = new McpServer({ name: 'gh-projects-mcp', version: SERVER_VERSION });
...
registerPreflightTool(server, { transport });
