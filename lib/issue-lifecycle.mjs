...
export function buildIssueCloseArgs(params) {
  const reason = params.closeReason ?? params.reason;
  return ['issue', 'close', String(params.number), '--repo', `${params.owner}/${params.repo}`, '--reason', issueCloseReason(reason)];
}
...
export function executeIssueMetadataEdit(ghImpl, params) {
  const before = readIssueForLifecycle(ghImpl, params.owner, params.repo, params.number);
  ghImpl(...buildIssueEditArgs(params));
  const after = readIssueForLifecycle(ghImpl, params.owner, params.repo, params.number);
  verifyIssueLifecycle(after, params);
  if (before.number !== after.number) throw new Error('Issue edit verification failed: issue identity changed.');
  return normalizeIssueLifecycleResult(after);
}

export function executeIssueStateChange(ghImpl, params) {
  const before = readIssueForLifecycle(ghImpl, params.owner, params.repo, params.number);
  if (params.state === 'closed') {
    assertConfirmed(params.confirm, 'close issue');
    ghImpl(...buildIssueCloseArgs(params));
  } else if (params.state === 'open' && before.state !== 'open') {
    ghImpl(...buildIssueReopenArgs(params));
  } else if (!['open', 'closed'].includes(params.state)) {
    throw new Error('Issue state must be open or closed.');
  }
  const after = readIssueForLifecycle(ghImpl, params.owner, params.repo, params.number);
  return verifyIssueLifecycle(after, params);
}
