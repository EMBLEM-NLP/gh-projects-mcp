...
export function registerPreflightTool(server, deps = {}) {
  ...
  // backendCapabilities() (lib/gql.mjs) returns the *active* backend's own
  // capabilities() summary — the request-scoped backend bound by the HTTP
  // transport (lib/request-context.mjs, #60) when one is active, otherwise
  // the process-wide stdio backend — whose `backend` field is exactly the
  // `.kind` the detector wants. Reshape it into the {kind, capabilities()}
  // the detector expects rather than importing github-backend.mjs a second
  // time.
  const getBackend = deps.getBackend ?? (() => {
    const caps = backendCapabilities();
    return { kind: caps.backend, capabilities: () => caps };
  });
  // Label only — which transport is reporting, not which tools/behavior it
  // gets (those are identical across transports). Threaded from
  // lib/server-factory.mjs's createMcpServer({ transport }) so gh_preflight
  // correctly reports "http" under the remote transport (#60) without a
  // second, parallel capability model.
  ...
  server.tool(
    ...
    makePreflightHandler({ gh, gql, getBackend, transport }),
  );
}
