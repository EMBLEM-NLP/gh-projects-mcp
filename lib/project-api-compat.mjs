...
export class ProjectApiCompat {
  ...
  fieldCreate(owner, number, name, dataType, optionNames = [], iterationConfiguration = null) {
  ...
  parseItemUrl(url) {
    ...
    const match = parsed.pathname.match(/^\/([^/]+)\/([^/]+)\/(issues|pull)\/(\d+)/);
    if (!match) throw new Error(`Unsupported issue/PR URL: ${url}`);
    const [, repoOwner, repo, kind, itemNumber] = match;
    return { repoOwner, repo, itemNumber: Number(itemNumber), type: kind === 'pull' ? 'PullRequest' : 'Issue' };
  }

  itemAdd(owner, number, url) {
    // POST .../projectsV2/{n}/items is a real, verified REST endpoint (GitHub's own
    // OpenAPI description) that takes owner/repo/number directly — no separate
    // node-ID resolution call needed, unlike the GraphQL path below it replaces
    // when available. GhCliBackend has no itemAddRest, so this falls through to
    // GraphQL there untouched.
    if (typeof this.backend.itemAddRest === 'function') {
      const { repoOwner, repo, itemNumber, type } = this.parseItemUrl(url);
      return this.backend.itemAddRest(owner, number, { type, repoOwner, repo, itemNumber });
    }
